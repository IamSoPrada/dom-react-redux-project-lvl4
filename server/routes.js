...
const buildState = (defaultState) => {
  ...
  const trashTalkChannelId = getNextId();
  const state = {
    channels: [
      { id: generalChannelId, name: 'general', removable: false },
      { id: randomChannelId, name: 'random', removable: false },
      { id: trashTalkChannelId, name: 'trash-talk', removable: true },
    ],
    messages: [],
    currentChannelId: generalChannelId,
    users: [
      { id: 1, username: 'admin', password: 'admin1' },
      { id: 2, username: 'manager', password: 'admin1' },
    ],
  };
...
export default (app, defaultState = {}) => {
  ...
  app.io.on('connect', (socket) => {
    ...
    socket.on('newMessage', (messageInfo, acknowledge = _.noop) => {
      const messageWithId = {
        ...messageInfo,
        id: getNextId(),
      };
